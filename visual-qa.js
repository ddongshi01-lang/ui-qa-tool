...
(function () {
  ...
  var AI_CHANGE_ICON_URLS = null;
  var AI_CHANGE_ICON_URLS_READY = false;
  var aiChangeIconUrlsPromise = null;
  ...
  function getAiChangeIconUrlsFromBridge() {
    if (AI_CHANGE_ICON_URLS_READY && AI_CHANGE_ICON_URLS) return Promise.resolve(AI_CHANGE_ICON_URLS);
    if (aiChangeIconUrlsPromise) return aiChangeIconUrlsPromise;
    aiChangeIconUrlsPromise = bridgeRequest("get-ai-change-icon-urls", {}, 3000)
      .then(function (response) {
        var iconUrls = response && response.iconUrls ? response.iconUrls : null;
        if (!iconUrls || !iconUrls.copy || !iconUrls.delete) {
          throw new Error("Missing ai change icon urls");
        }
        AI_CHANGE_ICON_URLS = iconUrls;
        AI_CHANGE_ICON_URLS_READY = true;
        return iconUrls;
      })
      .catch(function (err) {
        aiChangeIconUrlsPromise = null;
        AI_CHANGE_ICON_URLS = null;
        AI_CHANGE_ICON_URLS_READY = false;
        throw err;
      });
    return aiChangeIconUrlsPromise;
  }
  ...
  function resolveStrokeTarget(baseTarget) {
  ...
  function cloneStrokeConfig(config) {
    ...
    return {
      strokeColor: String(config.strokeColor || DEFAULT_STROKE_CONFIG.strokeColor),
      strokeAlpha: parsePercentValue(config.strokeAlpha),
      strokeWidth: Math.max(0, Math.round(parseFloat(config.strokeWidth) || 0)),
      strokeAlign: config.strokeAlign === "center" || config.strokeAlign === "outside" ? config.strokeAlign : "inside"
    };
  }

  function normalizeStrokeConfig(config, fallback) {
    var base = cloneStrokeConfig(fallback || DEFAULT_STROKE_CONFIG) || cloneStrokeConfig(DEFAULT_STROKE_CONFIG);
    var next = cloneStrokeConfig(config || {}) || {};
    return {
      strokeColor: toHexColor(next.strokeColor || base.strokeColor || DEFAULT_STROKE_CONFIG.strokeColor) || DEFAULT_STROKE_CONFIG.strokeColor,
      strokeAlpha: next.strokeAlpha === "" ? base.strokeAlpha : clamp(next.strokeAlpha == null ? base.strokeAlpha : next.strokeAlpha, 0, 100),
      strokeWidth: Math.max(0, Math.round(next.strokeWidth == null ? base.strokeWidth : next.strokeWidth)),
      strokeAlign: next.strokeAlign === "center" || next.strokeAlign === "outside" || next.strokeAlign === "inside"
        ? next.strokeAlign
        : base.strokeAlign
    };
  }

  function strokeAlignLabel(value) {
    return value === "center" ? "居中" : value === "outside" ? "外" : "内";
  }

  function normalizeStrokeAlignValue(value) {
    return value === "center" || value === "outside" || value === "inside" ? value : "inside";
  }

  function formatStrokeColorSummary(config) {
    var safe = normalizeStrokeConfig(config);
    return safe.strokeColor + (safe.strokeAlpha !== 100 ? " / " + safe.strokeAlpha + "%" : "");
  }
  ...
  function isPanelControlTarget(target) {
    return !!(
      target &&
      target.closest &&
      target.closest('input, textarea, select, option, [data-vqa-panel-control="true"]')
    );
  }

  function truncateDebugHtml(value, maxLen) {
  ...
  function debugNodeLabel(node) {
    ...
    if (node.nodeType === 1) {
      ...
      var role = node.getAttribute ? node.getAttribute("data-vqa-role") : "";
    ...
    return String(node.nodeName || typeof node);
  }

  function shouldLogStrokeAlignDebugEvent(event) {
    if (!event || !event.target) return false;
    if (event.target.closest && event.target.closest('[data-vqa-role="stroke-align"], [data-vqa-atom-cell="stroke-align"]')) {
      return true;
    }
    if (typeof event.composedPath === "function") {
      ...
      for (var i = 0; i < path.length; i++) {
        var node = path[i];
        if (node && node.getAttribute && (node.getAttribute("data-vqa-role") === "stroke-align" || node.getAttribute("data-vqa-atom-cell") === "stroke-align")) {
          return true;
        }
      }
    }
  ...
  function logStrokeAlignDebug(event) {
    if (!shouldLogStrokeAlignDebugEvent(event)) return;
    var selectEl = queryPanelSelector('[data-vqa-role="stroke-align"]');
    var pointEl = event && typeof event.clientX === "number" && typeof event.clientY === "number"
      ? document.elementFromPoint(event.clientX, event.clientY)
      : null;
    var rect = selectEl && typeof selectEl.getBoundingClientRect === "function" ? selectEl.getBoundingClientRect() : null;
    var computed = selectEl && window.getComputedStyle ? window.getComputedStyle(selectEl) : null;
    var path = typeof event.composedPath === "function" ? event.composedPath() : [];
    console.debug("[stroke-align-debug]", {
      type: event.type,
      target: debugNodeLabel(event.target),
      targetHtml: truncateDebugHtml(event.target && event.target.outerHTML ? event.target.outerHTML : "", 260),
      elementFromPoint: debugNodeLabel(pointEl),
      elementFromPointHtml: truncateDebugHtml(pointEl && pointEl.outerHTML ? pointEl.outerHTML : "", 260),
      composedPath: Array.isArray(path) ? path.slice(0, 12).map(debugNodeLabel) : [],
      selectRect: rect ? {
      ...
      disabled: !!(selectEl && selectEl.disabled),
  ...
  function getStrokeRuntimeMetaForTarget(targetEl) {
    ...
    return state.strokeRuntimeMeta && state.strokeRuntimeMeta.target === targetEl ? state.strokeRuntimeMeta.meta || null : null;
  }

  function setStrokeRuntimeMetaForTarget(targetEl, meta) {
    if (!targetEl || !meta) return;
    var map = state.strokeRuntimeMetaByTarget;
    if (map && typeof map.set === "function") {
      map.set(targetEl, meta);
    }
    state.strokeRuntimeMeta = { target: targetEl, meta: meta };
  }

  function clearStrokeRuntimeMetaForTarget(targetEl) {
    if (!targetEl) return;
    var map = state.strokeRuntimeMetaByTarget;
    if (map && typeof map.delete === "function") {
      map.delete(targetEl);
    }
    if (state.strokeRuntimeMeta && state.strokeRuntimeMeta.target === targetEl) {
      state.strokeRuntimeMeta = null;
    }
  }
  ...
  function buildStrokeShadowLayers(config) {
    var safe = normalizeStrokeConfig(config);
    var color = rgbaFromHexAndAlpha(safe.strokeColor, safe.strokeAlpha);
    if (!color || safe.strokeWidth <= 0) return [];
    if (safe.strokeAlign === "inside") {
      return ["inset 0 0 0 " + safe.strokeWidth + "px " + color];
    }
    if (safe.strokeAlign === "outside") {
      return ["0 0 0 " + safe.strokeWidth + "px " + color];
    }
    var insideWidth = Math.floor(safe.strokeWidth / 2);
    var outsideWidth = Math.ceil(safe.strokeWidth / 2);
    var layers = [];
    if (insideWidth > 0) layers.push("inset 0 0 0 " + insideWidth + "px " + color);
    if (outsideWidth > 0) layers.push("0 0 0 " + outsideWidth + "px " + color);
    return layers;
  }
  ...
  function recordStrokeLogicalChange(targetEl, prop, fromValue, toValue, options) {
    if (!targetEl || !prop) return;
    appendChangePatch(targetEl, prop, fromValue, toValue, options);
  }
  ...
  function applyStrokeConfig(targetEl, config, options) {
    ...
    if (options.updateModifiedProps !== false) {
      ...
      state.modifiedProps.strokeWidth = true;
      state.modifiedProps.strokeAlign = true;
      state.editedProps = state.modifiedProps;
    }
    if (!options.skipLogicalChangeRecord) {
      recordStrokeLogicalChange(targetEl, "strokeColor", previousConfig ? previousConfig.strokeColor : "", nextConfig.strokeColor, options);
      recordStrokeLogicalChange(targetEl, "strokeAlpha", previousConfig ? String(previousConfig.strokeAlpha) : "", String(nextConfig.strokeAlpha), options);
      recordStrokeLogicalChange(targetEl, "strokeWidth", previousConfig ? String(previousConfig.strokeWidth) : "", String(nextConfig.strokeWidth), options);
      recordStrokeLogicalChange(targetEl, "strokeAlign", previousConfig ? previousConfig.strokeAlign : "", nextConfig.strokeAlign, options);
    }
    return true;
  }
  ...
  function removeStrokeConfig(targetEl, options) {
    if (!targetEl || !targetEl.style) return false;
    var currentConfig = getPluginStrokeConfigForTarget(targetEl);
    restoreStrokeStyleSnapshot(targetEl);
    if (options && options.skipLogicalChangeRecord) return true;
    removeChangePatch(targetEl, "strokeColor");
    removeChangePatch(targetEl, "strokeAlpha");
    removeChangePatch(targetEl, "strokeWidth");
    removeChangePatch(targetEl, "strokeAlign");
    if (currentConfig) {
      clearStrokeModifiedProps();
    }
    return true;
  ...
  function buildStrokeConfigFromChangeItem(changeItem, valueKey) {
    ...
    patches.forEach(function (patch) {
      ...
      if (!config) config = normalizeStrokeConfig(getPluginStrokeConfigForTarget(changeItem && changeItem.elementRef) || DEFAULT_STROKE_CONFIG);
      if (patch.prop === "strokeColor") config.strokeColor = String(patchValue);
      if (patch.prop === "strokeAlpha") config.strokeAlpha = parsePercentValue(patchValue);
      if (patch.prop === "strokeWidth") config.strokeWidth = Math.max(0, Math.round(parseFloat(patchValue) || 0));
      if (patch.prop === "strokeAlign") config.strokeAlign = patchValue === "center" || patchValue === "outside" ? patchValue : "inside";
    });
  ...
  function buildQuickRecordTrackedValueMap(targetEl) {
    ...
    var strokeConfig = getPluginStrokeConfigForTarget(resolveStrokeTarget(targetEl));
    if (strokeConfig) {
      setQuickRecordComparableValue(map, "strokeColor", strokeConfig.strokeColor);
      setQuickRecordComparableValue(map, "strokeAlpha", String(strokeConfig.strokeAlpha));
      setQuickRecordComparableValue(map, "strokeWidth", String(strokeConfig.strokeWidth));
  ...
  function getQuickRecordPropKeyFromFieldId(fieldId) {
    ...
    if (/^strokeColor-alpha$/.test(fieldId)) {
      return "strokeAlpha";
    }
  ...
  function getQuickRecordDraftValue(targetEl, propKey) {
    ...
    if (propKey === "color" || propKey === "backgroundColor" || propKey === "strokeColor") {
      var colorTextFieldId = propKey + "-color-text";
      var alphaFieldId = propKey === "strokeColor" ? "strokeColor-alpha" : propKey + "-alpha";
      ...
      var fallbackText = "";
      var fallbackAlpha = "";
      if (propKey === "strokeColor") {
        var strokeConfig = getPluginStrokeConfigForTarget(resolveStrokeTarget(targetEl));
        fallbackText = strokeConfig ? String(strokeConfig.strokeColor || "").replace(/^#/, "") : "";
        fallbackAlpha = strokeConfig ? String(strokeConfig.strokeAlpha) : "";
      } else {
        fallbackText = getQuickRecordColorTextValue(style[propKey]);
        fallbackAlpha = getQuickRecordColorAlphaValue(style[propKey]);
      }
      var colorText = normalizeQuickRecordComparableValue(draftColorText != null ? draftColorText : fallbackText);
      return colorText ? "#" + colorText : "";
    }
    if (propKey === "strokeAlpha") {
      var draftStrokeAlpha = state.draftInputs["strokeColor-alpha"];
  ...
  function getAiChangeItemDisplayText(changeItem) {
    if (!changeItem) return "";
    var selector = String(changeItem.selector || "").trim();
    if (selector) return selector;
    var targetHint = changeItem.targetHint || {};
    if (typeof targetHint === "string") {
      var targetHintText = targetHint.trim();
      if (targetHintText) return targetHintText;
    }
    return (
      String(targetHint.label || "").trim() ||
      String(targetHint.className || "").trim() ||
      String(changeItem.targetName || "").trim()
    );
  }
  ...
  function ensureAiChangePanelStyles() {
    ...
    style.textContent =
      ...
      ".v12-ai-change-icon-btn{display:inline-flex;align-items:center;justify-content:center;width:28px;height:28px;padding:0;border:1px solid rgba(255,255,255,.12);border-radius:10px;background:rgba(255,255,255,.06);color:rgba(255,255,255,.8);cursor:pointer;transition:background-color 120ms ease,border-color 120ms ease,color 120ms ease,opacity 120ms ease,transform 120ms ease;}" +
      ".v12-ai-change-icon-btn:hover:not(:disabled){background:rgba(255,255,255,.1);border-color:rgba(255,255,255,.18);color:#fff;}" +
      ".v12-ai-change-icon-btn:active:not(:disabled){background:rgba(255,255,255,.14);transform:translateY(1px);}" +
      ".v12-ai-change-icon-btn:disabled{opacity:.42;cursor:not-allowed;}" +
      ".v12-ai-change-head-icon-btn{width:28px;height:28px;border-radius:8px;background:rgba(255,255,255,.06);border:0;}" +
      ".v12-ai-change-head-icon-btn:hover:not(:disabled){background:rgba(255,255,255,.12);border-color:transparent;}" +
      ".v12-ai-change-head-icon-btn:active:not(:disabled){background:rgba(255,255,255,.16);}" +
      ".v12-ai-change-icon{display:block;width:16px;height:16px;object-fit:contain;pointer-events:none;}" +
      ".v12-ai-change-head-icon-btn .v12-ai-change-icon{width:18px;height:18px;}" +
      ...
      ".v12-ai-change-body{display:flex;flex-direction:column;min-height:0;flex:1 1 auto;overflow:hidden;padding:14px;gap:12px;}" +
      ".v12-ai-change-list{display:flex;flex-direction:column;gap:10px;min-height:0;flex:1 1 auto;overflow-y:auto;overscroll-behavior:contain;padding:0 0 4px;}" +
      ...
      ".v12-ai-change-card-head{display:flex;align-items:flex-start;justify-content:space-between;gap:10px;max-width:100%;min-width:0;}" +
      ".v12-ai-change-card-side{display:flex;flex-direction:column;align-items:flex-end;gap:8px;flex:0 0 auto;flex-shrink:0;}" +
      ".v12-ai-change-card-actions{display:flex;align-items:center;gap:8px;flex-shrink:0;opacity:0;pointer-events:none;transition:opacity 120ms ease;}" +
      ...
      ".v12-ai-change-card-title{margin:0;max-width:100%;min-width:0;overflow:hidden;color:#fff;font:600 13px/1.4 -apple-system,BlinkMacSystemFont,Segoe UI,sans-serif;text-overflow:ellipsis;white-space:nowrap;}" +
      ".v12-ai-change-card-sub{margin:0;max-width:100%;min-width:0;overflow:hidden;color:rgba(255,255,255,.58);font:12px/1.45 -apple-system,BlinkMacSystemFont,Segoe UI,sans-serif;text-overflow:ellipsis;white-space:nowrap;}" +
      ...
      ".v12-ai-change-footer{display:flex;flex:0 0 auto;}" +
      ".v12-ai-change-copy-main{display:inline-flex;align-items:center;justify-content:center;width:100%;min-height:36px;padding:0 18px;border:0;border-radius:16px;background:rgba(255,255,255,.06);color:#fff;font:700 14px/1 -apple-system,BlinkMacSystemFont,Segoe UI,sans-serif;cursor:pointer;transition:background-color 120ms ease,opacity 120ms ease,transform 120ms ease;}" +
      ".v12-ai-change-copy-main:hover:not(:disabled){background:rgba(255,255,255,.1);}" +
      ".v12-ai-change-copy-main:active:not(:disabled){background:rgba(255,255,255,.14);transform:translateY(1px);}" +
      ".v12-ai-change-copy-main:disabled{opacity:.42;cursor:not-allowed;}" +
  ...
  function renderAiChangeIcon(src, alt) {
    if (!src) {
      return '<span aria-hidden="true" style="display:block;width:16px;height:16px;"></span>';
    }
    return '<img class="v12-ai-change-icon" src="' + esc(src) + '" alt="' + esc(alt || "") + '" draggable="false">';
  }
  ...
  function buildAiChangePanelHtml(changes) {
    ...
    var aiIcons = AI_CHANGE_ICON_URLS_READY && AI_CHANGE_ICON_URLS ? AI_CHANGE_ICON_URLS : null;
    var copyIconHtml = renderAiChangeIcon(aiIcons && aiIcons.copy, "");
    var deleteIconHtml = renderAiChangeIcon(aiIcons && aiIcons.delete, "");
    var bodyHtml = "";
    if (!count) {
      bodyHtml =
      ...
      bodyHtml = '<div class="v12-ai-change-list">' + items.map(function (changeItem) {
        ...
        return (
          ...
          '<p class="v12-ai-change-card-title">' + esc(getAiChangeItemDisplayText(changeItem)) + "</p>" +
          "</div>" +
          '<div class="v12-ai-change-card-side">' +
          '<div class="v12-ai-change-card-actions">' +
          '<button type="button" class="v12-ai-change-icon-btn" data-v12-action="copy-ai-change-item" data-vqa-action="copy-ai-change-item" data-change-item-id="' + esc(changeItem.id || "") + '" data-change-id="' + esc(changeItem.id || "") + '" aria-label="复制该项 Prompt">' + copyIconHtml + "</button>" +
          '<button type="button" class="v12-ai-change-icon-btn" data-v12-action="delete-ai-change-item" data-vqa-action="delete-ai-change-item" data-change-item-id="' + esc(changeItem.id || "") + '" data-change-id="' + esc(changeItem.id || "") + '" aria-label="删除并恢复该项修改">' + deleteIconHtml + "</button>" +
          "</div>" +
          "</div>" +
          "</div>" +
          '<p class="v12-ai-change-card-sub">' + esc(String(patchCount)) + " 项改动</p>" +
    ...
    return (
      ...
      '<button type="button" class="v12-ai-change-icon-btn v12-ai-change-head-icon-btn" data-v12-action="clear-ai-change-draft" data-vqa-action="clear-all-ai-changes" aria-label="清空 AI 修改"' + (hasChanges ? "" : " disabled") + ">" + deleteIconHtml + "</button>" +
      '<button type="button" class="v12-ai-change-close" data-v12-action="close-ai-change-panel" aria-label="关闭 AI 修改面板">×</button>' +
      "</div>" +
      "</div>" +
      '<div class="v12-ai-change-body">' +
      bodyHtml +
      '<div class="v12-ai-change-footer">' +
      '<button type="button" class="v12-ai-change-copy-main" data-v12-action="copy-ai-change-prompt" aria-label="复制 AI 修改提示词"' + (hasChanges ? "" : " disabled") + ">复制 Prompt</button>" +
      "</div>" +
      "</div>" +
      "</div>"
    );
  ...
  function renderAiChangePanel() {
    ...
    if (!AI_CHANGE_ICON_URLS_READY || !AI_CHANGE_ICON_URLS) {
      getAiChangeIconUrlsFromBridge()
        .then(function () {
          if (isInspectorAlive() && state.v12.aiChangePanelOpen) {
            state.v12.aiChangeRenderSignature = "";
            schedule();
          }
        })
  ...
  function applyChangePatchWithoutRecord(targetEl, patch) {
    ...
    if (isStrokeLogicalProp(patch.prop)) {
      var currentConfig = getPluginStrokeConfigForTarget(targetEl) || DEFAULT_STROKE_CONFIG;
      var nextConfig = cloneStrokeConfig(currentConfig) || cloneStrokeConfig(DEFAULT_STROKE_CONFIG);
      if (patch.prop === "strokeColor") nextConfig.strokeColor = normalizePromptValue(patch.to, DEFAULT_STROKE_CONFIG.strokeColor);
  ...
  function restorePatchWithoutRecord(targetEl, patch) {
    if (!targetEl || !patch || !patch.prop) return false;
    if (isStrokeLogicalProp(patch.prop)) {
      var hasAnyFromValue = false;
      var currentConfig = getPluginStrokeConfigForTarget(targetEl) || DEFAULT_STROKE_CONFIG;
      var nextConfig = cloneStrokeConfig(currentConfig) || cloneStrokeConfig(DEFAULT_STROKE_CONFIG);
      if (patch.from != null && patch.from !== "") hasAnyFromValue = true;
      if (patch.prop === "strokeColor") nextConfig.strokeColor = normalizePromptValue(patch.from, DEFAULT_STROKE_CONFIG.strokeColor);
      if (patch.prop === "strokeAlpha") nextConfig.strokeAlpha = parsePercentValue(patch.from);
      if (patch.prop === "strokeWidth") nextConfig.strokeWidth = Math.max(0, Math.round(parseFloat(patch.from) || 0));
      if (patch.prop === "strokeAlign") nextConfig.strokeAlign = patch.from === "center" || patch.from === "outside" ? patch.from : "inside";
      if (!hasAnyFromValue) {
        restoreStrokeStyleSnapshot(targetEl);
        return true;
      }
  ...
  function previewFieldValue(prop, rawValue) {
    var el = getEditableTargetEl();
    if (!el) return;
    if (prop === "strokeWidth" || prop === "strokeAlign") {
      var strokeTarget = resolveStrokeTarget(el);
      var strokeConfig = getPluginStrokeConfigForTarget(strokeTarget) || DEFAULT_STROKE_CONFIG;
      applyStrokeConfig(strokeTarget, {
        strokeColor: strokeConfig.strokeColor,
        strokeAlpha: strokeConfig.strokeAlpha,
        strokeWidth: prop === "strokeWidth" ? Math.max(0, Math.round(parseFloat(rawValue) || 0)) : strokeConfig.strokeWidth,
        strokeAlign: prop === "strokeAlign" ? String(rawValue || "inside") : strokeConfig.strokeAlign
      }, {
  ...
  function applyColorWithAlpha(prop, hexValue, alphaValue, targetOverride) {
    ...
    if (prop === "strokeColor") {
      var strokeTarget = resolveStrokeTarget(targetOverride || getEditableTargetEl());
      var strokeConfig = getPluginStrokeConfigForTarget(strokeTarget) || DEFAULT_STROKE_CONFIG;
      applyStrokeConfig(strokeTarget, {
        strokeColor: hexValue,
        strokeAlpha: alphaValue,
  ...
  function commitFieldDraft(target, options) {
    ...
    if (!rawValue) {
      if (prop === "strokeWidth") {
        target.value = String(DEFAULT_STROKE_CONFIG.strokeWidth);
        previewFieldValue(prop, DEFAULT_STROKE_CONFIG.strokeWidth);
        clearEditingState();
        if (!options.silent) schedule();
        return true;
      }
    ...
    if (prop === "strokeWidth") {
      var normalizedStrokeWidth = String(Math.max(0, Math.round(parseFloat(rawValue) || 0)));
      target.value = normalizedStrokeWidth;
      previewFieldValue(prop, normalizedStrokeWidth);
      clearEditingState();
      if (!options.silent) schedule();
      return true;
    }
  ...
  function onPanelChange(e) {
    ...
    if (target.matches && target.matches('[data-vqa-role="stroke-align"]')) {
      ...
      var nextAlign = normalizeStrokeAlignValue(target.value);
      target.value = nextAlign;
      state.editingFieldId = "strokeAlign";
      state.draftInputs.strokeAlign = nextAlign;
      var strokeConfig = getPluginStrokeConfigForTarget(strokeTarget) || DEFAULT_STROKE_CONFIG;
      applyStrokeConfig(strokeTarget, {
        strokeColor: strokeConfig.strokeColor,
        strokeAlpha: strokeConfig.strokeAlpha,
        strokeWidth: strokeConfig.strokeWidth,
        strokeAlign: nextAlign
      }, {
        source: "panel-change"
      });
      clearEditingState();
  ...
  function topbarCloseEntryHtml() {
    return topbarButtonHtml("toggle-collapse", "收起", TOPBAR_ICON_URLS && TOPBAR_ICON_URLS.close, false)
      .replace('class="v12-topbar-btn v12-topbar-icon-btn"', 'class="v12-topbar-btn v12-topbar-icon-btn v12-topbar-close-entry"');
  }
  ...
  function ensureTopbarStyles() {
    ...
    style.textContent =
      ...
      ".v12-topbar-drawer-slot{display:flex;align-items:center;justify-content:flex-end;gap:8px;margin-left:auto;flex:0 0 auto;min-width:80px;visibility:visible;opacity:1;transform:translateX(0) scale(1);transition:opacity 160ms ease,transform 180ms cubic-bezier(0.2,0,0,1),gap 180ms cubic-bezier(0.2,0,0,1);}" +
      ...
      ".v12-topbar-close-entry{width:36.207px;height:36.207px;}" +
      ".v12-topbar-shell.is-collapsed .v12-topbar-drawer-slot{min-width:36.207px;gap:0;}" +
      ".v12-topbar-shell.is-collapsed .v12-topbar-close-entry{width:0;opacity:0;visibility:hidden;pointer-events:none;overflow:hidden;}" +
  ...
  function syncTopbarIconSources() {
    ...
    var iconMap = {
      ...
      "toggle-collapse": TOPBAR_ICON_URLS && TOPBAR_ICON_URLS.close
    };
  ...
  function ensureTopbarDom() {
    ...
    topbarDom.toggleCollapseBtn = topbar.querySelector('[data-v12-action="toggle-collapse"]');
  ...
  function syncTopbar() {
    ...
    syncTopbarButton(topbarDom.toggleCollapseBtn, false, state.v12.topbarPressedAction === "toggle-collapse");
    if (topbarDom.recordRegionBtn) {
      topbarDom.recordRegionBtn.setAttribute("data-has-dot", getAiChangeCount() > 0 ? "true" : "false");
    }
  ...
  function handleTopbarAction(action, target, e) {
    ...
    if (action === "mode-select") {
      ...
      branch = "toggle-collapse";
      closeFeedbackPopover({ schedule: false }, "topbar-toggle-collapse");
      toggleCollapsed();
    }
    ...
    return (
      ...
      panelAtomGrid([
        ...
        panelIconValueCell("", "strokeWidth", String(normalizedStrokeConfig.strokeWidth), {
          kind: "stroke-width",
          flex: "1 1 auto",
          slotName: "stroke-border",
          iconSize: "30px",
          iconWidth: "30px",
          iconBg: "transparent",
          iconColor: "transparent",
          iconRadius: "0px",
          inputFont: "400 14px/1.25 -apple-system,BlinkMacSystemFont,Segoe UI,sans-serif",
          inputTextAlign: "left"
        })
      ], { gap: "6px" }) +
      "</div>"
  ...
  function renderSelectedStrokeSection(el) {
    var strokeTarget = resolveStrokeTarget(el);
    if (!strokeTarget) return "";
    if (!getPluginStrokeConfigForTarget(strokeTarget)) return "";
    return section(
      "描边",
      [renderStrokeConfigControls(strokeTarget)],
      "",
      '<button type="button" data-action="remove-stroke" aria-label="删除描边" title="删除描边" style="border:0;background:transparent;color:rgba(255,255,255,.66);font:500 12px/1 -apple-system,BlinkMacSystemFont,Segoe UI,sans-serif;cursor:pointer;padding:0;">删除</button>'
    );
  }
  ...
  function getSelectedPanelMaxHeightPx() {
    var viewportHeight = Math.max(window.innerHeight || 0, 0);
    if (!viewportHeight) return 0;
    var preferredHeight = Math.max(160, viewportHeight - 96);
    var availableHeight = Math.max(120, viewportHeight - 16);
  ...
  function renderSelectedPanel(el) {
    ...
    var panelMaxHeight = getSelectedPanelMaxHeightPx();
    body.innerHTML =
  ...
  function onClick(e) {
    ...
    if (tooltip.contains(e.target) && isPanelControlTarget(e.target)) return;
  ...
  function destroy(reason) {
    ...
    tooltip.removeEventListener("pointerdown", logStrokeAlignDebug, true);
    tooltip.removeEventListener("mousedown", logStrokeAlignDebug, true);
    tooltip.removeEventListener("click", logStrokeAlignDebug, true);
    tooltip.removeEventListener("change", logStrokeAlignDebug, true);
  ...
  tooltip.addEventListener("pointerdown", logStrokeAlignDebug, true);
  tooltip.addEventListener("mousedown", logStrokeAlignDebug, true);
  tooltip.addEventListener("click", logStrokeAlignDebug, true);
  tooltip.addEventListener("change", logStrokeAlignDebug, true);
